...
class PlaneModelViewer(
    ...
    private fun addDetachListener(view: android.view.View) {
        view.addOnAttachStateChangeListener(object : android.view.View.OnAttachStateChangeListener {
            ...
            override fun onViewDetachedFromWindow(v: android.view.View) {
                ...
                engine.destroyView(this@PlaneModelViewer.view)
    ...
    override fun onTouch(view: android.view.View, e: MotionEvent): Boolean {
        this.view.pick(e.x.toInt(), e.y.toInt(), pickingHandler) { result ->
            if (result.renderable == planEntity) {
                val y: Float = e.y
                ...
                when (e.action) {
                    MotionEvent.ACTION_MOVE -> {
                        ...
                        val tm = engine.transformManager
                        val transform = translation(Float3(0f, normalizedY, -3f))
                        tm.setTransform(tm.getInstance(planEntity!!), transpose(transform).toFloatArray())
                    }
                }
            }
        }
