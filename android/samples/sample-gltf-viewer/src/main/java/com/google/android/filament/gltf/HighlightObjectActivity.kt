...
class HighlightObjectActivity : Activity() {
    ...
    private lateinit var selectedMatInstance: MaterialInstance
...
//        modelViewer.loadModelGltfAsync(buffer) { uri -> readCompressedAsset("models/$uri") }
    ...
    private var selectedRenderableEntity: Int? = null
    private var actualMatInstance: MaterialInstance? = null
    ...
    inner class SingleTapListener : GestureDetector.SimpleOnGestureListener() {
        override fun onSingleTapUp(event: MotionEvent): Boolean {
            modelViewer.view.pick(
                ...
                selectedRenderableEntity?.let {
                    modelViewer.engine.renderableManager.run {
                        setMaterialInstanceAt(getInstance(it), 0, actualMatInstance!!)
                        selectedRenderableEntity = null
                        actualMatInstance = null
                    }
                    return@pick
                }
                if (modelViewer.asset!!.renderableEntities.contains(it.renderable)) {
                    modelViewer.engine.renderableManager.run {
                        logg("getting actual matintsance")
                        actualMatInstance = getMaterialInstanceAt(
                            getInstance(it.renderable), 0
                        )
                    ...
                    selectedRenderableEntity = it.renderable
                }
            }
    ...
    private fun buildMaterial() {
        ...
        val matPackage = MaterialBuilder()
            ...
            .material("void material(inout MaterialInputs material) {\n" +
                ...
                "    material.baseColor.rgb = vec3(1.0, 1.0, 0);\n" +
                "}\n")
        ...
        if (matPackage.isValid) {
            ...
            selectedMaterial = Material.Builder().payload(buffer, buffer.remaining()).build(modelViewer.engine)
//            selectedMatInstance = selectedMaterial.createInstance()
        }
