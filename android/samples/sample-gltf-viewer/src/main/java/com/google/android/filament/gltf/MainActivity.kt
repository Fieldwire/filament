    ...
    inner class SingleTapListener : GestureDetector.SimpleOnGestureListener() {
        override fun onSingleTapUp(event: MotionEvent): Boolean {
            ...
            modelViewer.asset?.let { asset ->
                val hit = asset.pick(modelViewer.view, event.x.toInt(), event.y.toInt()) // Updated usage: removed redundant asset parameter
                if (hit != null) {
                    val pickedName = asset.getName(hit.entity)
                    Log.v(
                        "Filament",
                        "Ray-picked entity=${hit.entity} name=${pickedName} tri=${hit.triangle}"
                    )
                } else {
