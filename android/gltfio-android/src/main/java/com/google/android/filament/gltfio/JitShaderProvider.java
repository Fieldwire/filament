...
/**
 * Builds materials on the fly using Filamat on Android (JIT shader path).
 *
 * <p>This class mirrors {@link UbershaderProvider} but compiles material
 * programs at runtime. Prefer ubershader in production for predictable frame times.
 * Use this for parity testing or when runtime compilation is acceptable.</p>
 */
public class JitShaderProvider implements MaterialProvider {
    ...
    /**
     * Constructs a JIT shader provider using the supplied {@link Engine}.
     *
     * @param engine the engine used to create materials
     * @param optimize if true, Filamat will optimize shaders (slower compile)
     * @param variantFilters optional list of variant filter names, e.g.,
     *                      {"directionalLighting","dynamicLighting","shadowReceiver"}
     */
    public JitShaderProvider(Engine engine, boolean optimize, @Nullable String[] variantFilters) {
        long nativeEngine = engine.getNativeObject();
        mNativeObject = nCreateJitShaderProvider(nativeEngine, optimize, variantFilters);
    }

    /**
     * Convenience constructor with sensible defaults.
     * Optimize=false, variantFilters = { directionalLighting, dynamicLighting, shadowReceiver }.
     */
    public JitShaderProvider(Engine engine) {
        this(engine, false, new String[] {});
    }
    ...
    public boolean needsDummyData(int attrib) {
        // JIT provider does not require dummy attributes.
        return false;
    }
    ...
    private static native long nCreateJitShaderProvider(long nativeEngine, boolean optimize, String[] variantFilters);
