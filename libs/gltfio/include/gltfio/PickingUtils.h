...
// and the asset root, using the asset's Engine/TransformManager.
// Returns the number of renderable transforms updated.
inline size_t updateTinybvhPickingTransforms(filament::gltfio::FilamentAsset* asset) {
    if (!asset) return 0;
    filament::gltfio::TinyBVHPickingRegistry* reg = asset->getTinyBVHPickingRegistry();
    if (!reg) return 0;
    filament::Engine* engine = asset->getEngine();
    if (!engine) return 0;
    auto& tcm = engine->getTransformManager();
